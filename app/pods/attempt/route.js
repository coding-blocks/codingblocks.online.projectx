...
export default Route.extend(AuthenticatedRouteMixin, {
  ...
  model(params, transition) {
    ...
    return this.store.findRecord('runAttempt', params.runAttemptId, { reload: true }).catch(err => {
      if (err instanceof DS.AdapterError) {
        ...
        return this.api
          .request('/users/correctRunAttempt/' + params.runAttemptId)
          .then(result => {
            // replace url param with corrected runAttemptId
            const url = this.get('router.currentURL').replace(
              /player\/\d*\//g,
              'player/' + result.id + '/',
            );
