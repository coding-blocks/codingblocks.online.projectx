...
export default class CodeChallengeComponent extends Component {
  ...
  @computed('code.content.id', 'runAttempt.{id,doubts}')
  get relatedPendingDoubt() {
    ...
    return runAttempt.doubts.find(
      doubt =>
        doubt.get('content.id') == this.code.get('content.id') &&
        ['PENDING', 'ACKNOWLEDGED'].includes(doubt.get('status')),
    );
  }
  ...
  didReceiveAttrs() {
    ...
    if (
      this.get('problemJsonApiPayload') &&
      +this.get('problemJsonApiPayload.data.id') === code.get('hbProblemId')
    ) {
      return;
    }
    this.get('api')
  ...
  @restartableTask runCodeTask = function*(config) {
    ...
    const status = yield this.taskPoller.performPoll(
      () =>
        this.get('api').request('code_challenges/status/' + submissionId, {
          method: 'GET',
        }),
    ...
    this.get('api')
      ...
      .then(async result => {
        ...
        later(async () => {
          ...
          if (
            (await problem.get('hasLatestSubmissionPassed')) &&
            (await problem.get('mostSuccessfullSubmission.score')) == 100
          ) {
  ...
  @action
  fetchEditorial() {
    this.get('fetchEditorialTestcases')
      .perform('editorials')
      .then(response => {
    ...
    this.get('fetchEditorialTestcases')
      ...
      .then(response => {
        const testcases = response.data.map(t => {
          return this.get('store').createRecord('testcase', {
  ...
  @restartableTask fetchEditorialTestcases = function*(which) {
    try {
      ...
      const run = this.get('run');
      const code = this.get('code');
      return yield this.get('api').request(
        `code_challenges/` +
          which +
          `?contest_id=${run.get('contestId')}&p_id=${code.get('hbProblemId')}&force=true`,
      );
    } catch (err) {
      switch (which) {
        case 'editorials':
          ...
          break;
        case 'testcases':
          this.set('err', 'Testcase unlock is blocked for this problem');
          break;
      }
    }
  };
