    ...
    return this.store
      ...
      .then(runAttempts => {
        ...
        if (isNone(runAttempt)) {
          ...
          return this.api
            ...
            .catch(err => {
              if (err.status == 400 && err.payload.err == 'TRIAL_WITHOUT_MOBILE') {
                // trial creation denied because user has no mobile number
          ...
          return this.store.findRecord('run-attempt', runAttempt.get('id'), {
            reload: true,
          });
        }
      })
      ...
      .then(async runAttempt => {
        await this.api
          .request('courses/' + runAttempt.get('run.course.id') + '/rating', {
            method: 'GET',
          })
          .then(rating => {
